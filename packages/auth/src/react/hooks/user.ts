import type { UserQuery } from '@core/api';
import { UserDocument } from '@core/api';
import { useSession } from '@core/auth/react/hooks/session';
...
export function useUser<Required extends boolean = false>(
  ...
  const [{ data, fetching, error }, fetch] = useQuery(UserDocument, {
