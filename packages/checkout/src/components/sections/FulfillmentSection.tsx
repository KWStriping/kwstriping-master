...
import { UpdateCheckoutFulfillmentMethodDocument } from '@core/api';
...
import { useFormDebouncedSubmit } from '@core/ui/hooks';
import { useShopSettings } from '@core/ui/providers';
import { getFormattedMoney } from '@core/ui/utils';
...
export const FulfillmentSection: FC<CommonCheckoutSectionProps> = ({ checkout, className }) => {
  ...
  const getSubtitle = ({ min, max }: { min?: number | null; max?: number | null }) => {
    ...
    if (min) return t('checkout.minDays', 'In {{min}}+ business days', { min: min.toString() });
    if (max)
      return t('checkout.maxDays', 'Within {{max}} business days', { max: max.toString() });
    return '';
  };
  ...
  return (
    <CheckoutSection
      ...
      header={t('checkout.fulfillmentMethods.header', 'Delivery methods')}
      className={className}
      validate={validate}
    >
      {!shippingAddress ? (
        <Typography>
          {t(
            'checkout.fulfillmentMethods.noShippingAddress',
              ...
              <div className={'col-span-12'}>
                <Controller
                  ...
                  render={({ field: { onChange, value } }) => (
                    <DateTimeField
                      name="fulfillmentDeadline"
                      label={t('checkout.fulfillmentDeadline', 'Fulfillment deadline')}
                      value={value ?? ''}
                      onChange={(newValue) => {
                        onChange(newValue);
                        try {
              ...
              <Controller
                ...
                render={({ field: { onChange } }) => (
                  <SelectBoxGroup
                    ...
                    onChange={onChange}
                    label={t('checkout.fulfillmentSection.methods', '')}
        ...
        <div>
          {enableFulfillmentDeadline && (
            <div className="flex flex-wrap justify-between gap-8 mt-2">
              <Typography>
                {t('checkout.fulfillmentSection.fulfillmentDeadline', 'Fulfillment deadline')}
              </Typography>
          ...
          <div className="flex flex-wrap justify-between gap-8 mt-2">
            <Typography>
              {t('checkout.fulfillmentSection.shippingMethod', 'Fulfillment method')}
            </Typography>
            <div>
          ...
          {!!selectedMethod?.price.amount && displayPrices && (
            <div className="flex justify-between">
              <Typography className="mb-2">
                {t('checkout.fulfillmentSection.price', 'Price')}
              </Typography>
              <Typography className="mb-2">
