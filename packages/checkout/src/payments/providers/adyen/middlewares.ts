...
import type { PrivateSettingsValues } from '@core/checkout/types/api';
import { adyenProviderSettingIDs } from '@core/checkout/types/payments';
...
export const withAdyenWebhookCredentials: Middleware = (handler) => async (request) => {
  const [error, settings] = [] as any; // TODO: await unpackPromise(getPrivateSettings(API_URL, false));
