...
function getPosition<T extends HTMLElement>(anchor?: T | null): Position | undefined {
  if (anchor) {
    return {
      x: anchor?.scrollLeft,
      y: anchor?.scrollTop,
...
export function isScrolledToBottom(anchor: HTMLElement, position: Position, offset = 0) {
  return !!anchor && position
    ? position.y + anchor.clientHeight + offset >= anchor.scrollHeight
    : undefined;
}

export function isScrolledToTop(anchor: HTMLElement, position: Position, offset = 0) {
...
export function useElementScroll<T extends HTMLElement>(): UseElementScroll<T> {
  ...
  useEffect(() => {
    if (anchorEl) {
      const recalcScrollPos = throttle(() => setScroll(getPosition(anchorEl)), 100);
  ...
  const setAnchor = useCallback((el: T | null) => {
    setAnchorEl(el);
  }, []);
