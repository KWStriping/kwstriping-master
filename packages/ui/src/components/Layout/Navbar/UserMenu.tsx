import type { UserDetailsFragment } from '@core/api';
      ...
      <div className={styles['user-menu'] ?? ''}>
        <MenuItem className={styles['user-menu-item'] ?? ''}>
          <Link href={paths.accountPreferences()} tabIndex={0}>
            <Typography>{t('menuAccountPreferences', 'Account preferences')}</Typography>
          </Link>
        </MenuItem>
        <Divider />
        <MenuItem className={styles['user-menu-item'] ?? ''}>
        ...
        >
          <Typography>{t('logOut', 'Log out')}</Typography>
        </MenuItem>
      </div>
    </div>
  );
