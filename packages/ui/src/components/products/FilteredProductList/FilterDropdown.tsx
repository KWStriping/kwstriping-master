...
export function FilterDropdown({
  ...
  return (
    <Menu as="div" className="relative inline-block text-left">
      ...
      >
        <Menu.Items className="focus:outline-none absolute left-0 origin-top-right bg-white shadow-lg ring-1 ring-black ring-opacity-5 z-10">
          {options?.map((option) => (
            <Menu.Item key={option.id}>
              {({ active }) => (
                <button
                  ...
                  className={clsx(
                    'group flex w-full px-2 py-2 text-base border-2',
                    active ? 'border-brand text-brand' : 'border-transparent text-gray-900'
                  )}
                  data-testid={`filterValue${option.label}`}
                >
