...
import Draggable from '@dashboard/oldSrc/icons/Draggable';
import type { RecursiveMenuItem } from '@dashboard/oldSrc/navigation/types';
...
const Placeholder: FC = (props) => {
  ...
  return (
    <Paper className={styles.row ?? ''} elevation={0}>
      <Typography>{t('dashboard.wZfNK', 'Add new menu item to begin creating menu')}</Typography>
    </Paper>
  );
};
    ...
    >
      <Paper className={styles.row ?? ''} elevation={0}>
        ...
        <div className={styles.nodeActions ?? ''}>
          <Button onClick={node.onClick}>{t('dashboard.show', 'Show')}</Button>
...
const MenuItems: FC<MenuItemsProps> = ({
  ...
  return (
    <Card>
      <CardTitle
        title={t('dashboard.menuItemsHeader', 'Menu Items')}
        toolbar={
      ...
      <CardActions className={styles.actions ?? ''}>
        <Button onClick={onItemAdd} data-test-id="create-new-menu-item">
          {t('dashboard.createNewMenuItem', 'Create new item')}
        </Button>
      </CardActions>
    </Card>
  );
