...
import type { MenuDetailsFragment, MenuErrorFragment } from '@core/api/graphql';
...
const MenuDetailsPage: FC<MenuDetailsPageProps> = ({
  ...
  return (
    <Form confirmLeave initial={initialForm} onSubmit={handleSubmit}>
      {({ change, data, submit }) => (
        <Container>
          <Backlink href={'/navigation'}>{t('dashboard.navigation', 'Navigation')}</Backlink>
          <Grid variant="inverted">
            <div>
              <Typography variant="h5">{t('dashboard.navigation', 'Navigation')}</Typography>
              <Typography>
                <>
                  {t(
                    'dashboard.54eoT',
