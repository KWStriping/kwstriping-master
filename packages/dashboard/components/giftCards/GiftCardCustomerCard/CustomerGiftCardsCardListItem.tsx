...
import type { CustomerGiftCardFragment } from '@core/api/graphql';
import { giftCardsListTableMessages } from '@dashboard/oldSrc/giftCards/GiftCardsList/messages';
import useGiftCardActivateToggle from '@dashboard/oldSrc/giftCards/GiftCardUpdate/GiftCardUpdatePageHeader/hooks/useGiftCardActivateToggle';
...
const CustomerGiftCardsCardListItem: FC<CustomerGiftCardsCardListItemProps> = ({ giftCard }) => {
  ...
  const getMenuItems = (): CardMenuItem[] => {
    const items = [
      {
        label: t('dashboard.deleteLabel', 'Delete'),
    ...
    const statusButton = isActive
      ? {
          label: t('dashboard.disableLabel', 'Deactivate'),
      ...
      : {
          label: t('dashboard.enableLabel', 'Activate'),
