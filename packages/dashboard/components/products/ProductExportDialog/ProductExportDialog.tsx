...
import type { Step } from '@dashboard/components/core/CreatorSteps';
import makeCreatorSteps from '@dashboard/components/core/CreatorSteps';
...
import getExportErrorMessage from '@dashboard/oldSrc/utils/errors/export';
...
const ProductExportDialog: FC<ProductExportDialogProps> = ({
  ...
  const exportScopeLabels = {
    allItems: t('dashboard.tUXnK', 'All products ({number})', {
      number: productQuantity.all || '...',
    }),
    selectedItems: t('dashboard.EZ463', 'Selected products ({number})', {
  ...
  return (
    <Dialog onClose={onClose} open={open} maxWidth="sm" fullWidth>
      <>
        <DialogTitle>{t('dashboard.title', 'Export Information')}</DialogTitle>
        <DialogContent>
        ...
        <DialogActions>
          ...
          {step === ProductExportStep.Settings && (
            <ConfirmButton
              ...
              onClick={submit}
            >
              {t('dashboard.onfirmButtonLabel', 'export products')}
            </ConfirmButton>
          )}
        </DialogActions>
      </>
