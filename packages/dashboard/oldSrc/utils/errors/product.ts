  ...
  if (err) {
    switch (err.code) {
      case ProductErrorCode.AttributeAlreadyAssigned:
        return t(
          'dashboard.attributeAlreadyAssigned',
...
export function getProductAttributeErrorMessage(
  ...
  if (err) {
    switch (err.code) {
      case ProductErrorCode.Unique:
        return t('dashboard.variantUnique', messages.variantUnique.defaultMessage);
      default:
