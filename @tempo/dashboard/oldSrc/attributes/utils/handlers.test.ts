...
interface CreateAttribute {
  ...
  initialValue?: ValueDetailsFragment[];
  availableValues?: ValueDetailsFragment[];
  value?: string;
}
...
const createPlainTextAttribute = (value: string) =>
  createAttribute({ inputType: AttributeInputType.PLAIN_TEXT, value });

const createRichTextAttribute = (value: string) =>
  createAttribute({ inputType: AttributeInputType.RICH_TEXT, value });

const createDateAttribute = (value: string) =>
  createAttribute({ inputType: AttributeInputType.DATE, value });

const createDateTimeAttribute = (value: string) =>
  createAttribute({ inputType: AttributeInputType.DATE_TIME, value });

const createSwatchAttribute = (value: string) =>
  createAttribute({ inputType: AttributeInputType.SWATCH, value });

const createNumericAttribute = (value: string) =>
  createAttribute({ inputType: AttributeInputType.NUMERIC, value });

const createFileAttribute = (value: string) =>
  createAttribute({ inputType: AttributeInputType.FILE, value });
...
describe('Sending only changed attributes', () => {
  ...
  describe('works with file attributes', () => {
    it('removes existing image (img -> null)', () => {
      ...
      // Files are deleted by using ValueDetele mutation
    ...
    it('adds new image (null -> img)', () => {
      ...
      const uploadUrl = 'http://some-url.com/media/file_uploads/bob.jpg';
      const result = prepareAttributesInput({
        attributes: [attribute],
        prevAttributes: [prevAttribute],
        updatedFileAttributes: [
    ...
    it('replaces existing image (bob.jpg -> juice.png)', () => {
      ...
      const uploadUrl = 'http://some-url.com/media/file_uploads/juice.jpg';
      const result = prepareAttributesInput({
        attributes: [attribute],
        prevAttributes: [prevAttribute],
        updatedFileAttributes: [
