...
describe("Getting items", () => {
  ...
  it("Properly returns an nested item by path", () => {
    expect(getMenuItemByPath(validMenu, [3, 0])).toMatchObject(validMenu[3].children[0]);
  });

  it("Properly returns an item from root by value", () => {
    expect(getMenuItemByValue(validMenu, validMenu[1].value)).toMatchObject(validMenu[1]);
  });

  it("Properly returns an nested item by value", () => {
    expect(getMenuItemByValue(validMenu, validMenu[3].children[0].value)).toMatchObject(
...
describe("Walk through menu", () => {
  ...
  it("Properly returns all nodes on the path to root", () => {
    expect(walkToRoot(validMenu, validMenu[3].children[0].value)).toMatchSnapshot();
  });
});
