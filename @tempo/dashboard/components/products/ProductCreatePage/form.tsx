...
import type { ChannelData, ChannelPriceArgs } from '@tempo/dashboard/oldSrc/channels/utils';
import { PRODUCT_CREATE_FORM_ID } from '@tempo/dashboard/oldSrc/products/ProductCreate/consts';
import type { ProductKlass } from '@tempo/dashboard/oldSrc/products/utils/data';
import { getAttributeInputFromProductKlass } from '@tempo/dashboard/oldSrc/products/utils/data';
import {
  createChannelsChangeHandler,
  createChannelsPriceChangeHandler,
  createProductKlassSelectHandler,
...
function useProductCreateForm(
  ...
  const defaultInitialFormData: ProductCreateFormData & Record<'productKlass', string> = {
    category: '',
  ...
  const attributes = useFormset<AttributeInputData>(
    opts.selectedProductKlass ? getAttributeInputFromProductKlass(opts.selectedProductKlass) : []
  );
  ...
  const handleProductKlassSelect = createProductKlassSelectHandler(
    opts.onSelectProductKlass,
    triggerChange
  );
  ...
  const data: ProductCreateData = {
    ...
    description: null,
    productKlass: opts.selectedProductKlass,
    stocks: stocks.data,
  };
  ...
  const isValid = () => {
    if (!data?.name || !data?.productKlass) {
      return false;
    }
    ...
    if (opts.selectedProductKlass?.hasVariants) {
      return true;
    }
  ...
  return {
    ...
    handlers: {
      ...
      selectProductKlass: handleProductKlassSelect,
