...
const byChannelColumn = (change: DatagridChange) => getColumnChannel(change.column);
...
const byNotNullPrice = (change: ReturnType<typeof dataGridChangeToFlatChannel>) =>
  change.price !== null;

export function getVariantChannelsInputs(data: DatagridChangeOpts, index: number) {
  return data?.updates
