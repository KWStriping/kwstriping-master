...
import {
  ...
  numberCell,
  textCell,
} from '@tempo/dashboard/components/core/Datagrid/cells';
import { emptyDropdownCellValue } from '@tempo/dashboard/components/core/Datagrid/DropdownCell';
import { numberCellEmptyValue } from '@tempo/dashboard/components/core/Datagrid/NumberCell';
import type { AvailableColumn } from '@tempo/dashboard/components/core/Datagrid/types';
import type {
  DatagridChange,
  DatagridChangeOpts,
...
export function getColumnData(
  ...
  if (['name', 'sku', 'trackInventory'].includes(name)) {
    return {
      ...common,
      title: m[name] ?? messages[name as keyof typeof messages] ?? sentenceCase(name),
    };
  }
  ...
  if (getColumnStock(name)) {
    return {
      ...
      group: m.dashboard_warehouses() ?? 'Warehouses',
    };
  }
  ...
  if (getColumnChannelAvailability(name)) {
    ...
    return {
      ...common,
      width: 80,
      title: m.dashboard_vailable() ?? 'Available',
      group: channel.name,
    };
  }
  ...
  if (getColumnAttribute(name)) {
    return {
      ...
      title: variantAttributes.find((attribute) => attribute.id === getColumnAttribute(name))
        ?.name,
      group: m.dashboard_attributes() ?? 'Attributes',
    };
  }
