...
export function useListStory(initialRowsPerPage = 2) {
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(initialRowsPerPage);
  const [selected, setSelected] = useState<string[]>([data[0].name, data[2].name]);

  const hasNextPage = (page + 1) * rowsPerPage < data?.length;
  const hasPreviousPage = page > 0;

  const isRowSelected = (name: string) => !!selected.find((selected) => selected === name);
