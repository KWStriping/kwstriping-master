...
export function change(
  ...
  assert(!!filterToUpdate, `Filter ${name} not found`);
  let update: Partial<FilterData>;
  if (filterToUpdate.options.multiple) {
    update = {
      values: value as string[],
    };
  } else if (filterToUpdate.options.type === FilterType.Range && opts.rangePart) {
    assert(!!filterToUpdate.values);
    update = {
      values:
        opts.rangePart === 'min'
          ? [value as string, filterToUpdate.values[1] as string]
          : [filterToUpdate.values[0] as string, value as string],
    };
  } else {
...
export function toggleRange(filterData: FilterData[], name: string): FilterData[] {
  return filterData.map((filter) =>
    filter.name === name
      ? {
          ...filter,
