...
import type { FilterElement, InvalidFilters } from './types';
import { FieldType, ValidationErrorCode } from './types';
...
export const extractInvalidFilters = function <T extends string>(
  ...
  return filtersDataStructure.reduce((invalidFilters, { name, multipleFields, dependencies }) => {
    ...
    if (!isFilterValid(filter)) {
      errors.push(ValidationErrorCode.ValueRequired);
    }
    ...
    if (dependencies?.length) {
      ...
      if (deps?.length) {
        errors.push(ValidationErrorCode.DependenciesMissing);
      }
    }
